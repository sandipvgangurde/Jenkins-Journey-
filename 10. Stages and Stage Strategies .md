...
When your pipeline includes more than just a single task (e.g., **Build → Test → Deploy**), you’ll want to **define multiple stages** for clarity and control.
...
This is the default behavior in Jenkins — stages run in sequence, one after the other.
...
Here’s how we can organize **sequential vs. parallel stages** with a simple **tree diagram**:
...
```
Pipeline 🌟
 ├── Build ➔ [Test 1] 💻 and [Test 2] 🧪
        ↘️ Only if `master` branch
```
...
```
[ Build on Ubuntu ] 🏗️  |  [ Build on Windows ] 🏗️
[ Test on Chrome ] 🧪    |  [ Test on Firefox ] 🧪
```

#### **b. Conditional Parallel Execution** 🔄
...
```
+---------------------+                   +------------------------+  
| Sequential Stages     | ➔ Sequential   |  Parallel Stages        |
...
# 🚀 **Summary of Stages & Strategies**

Here’s a quick overview of what we've covered:

| Concept               | Explanation                                       | Emoji   |
| --------------------- | ------------------------------------------------- | ------- |
| **Sequential Stages** | Stages that run one after another.                | 🚶‍♂️   |
| **Parallel Stages**   | Stages that run simultaneously to save time.      | 🏃‍♂️💨 |
| **Conditionals**      | Use the `when` block to conditionally run stages. | 🔄      |
| **Timeouts**          | Limit the duration of stages.                     | ⏰       |
| **Retries**           | Retry failed stages a set number of times.        | 🔁      |
| **Matrix**            | Run stages across multiple combinations.          | 📊      |
| **Post Actions**      | Actions that run after the stage completes.       | 🔚      |
